export default{
    ...
    methods: {
        ...
        openModal: function (modalName = 'ModalVue', title = false, callback = false, resetValidation = true) {
            if (title) {
            ...
            this.$validator.validate().then(valid => {
                if (valid || !checkValidation) {
                    ...
                    _this.axios({method: method, url: URL, data: formData, params: object}).then(function (response) {
                        ...
                        if (parseInt(returnData.status) === 5001) {
                            _this.$swal('error', returnData.message, 'Error');
                            // _this.$router.push({path:'/admin/dashboard'});
                            return;
                        }

                        if (parseInt(returnData.status) === 2000) {
                            if (model) {
                            ...
                            _this.$swal('success', returnData.message, 'Success');
                        }
                        if (parseInt(returnData.status) === 3000) {
                            _this.$swal('warning', returnData.message, 'Warning');
                            _this.assignValidationError(returnData.result);
                        }
                        if (parseInt(returnData.status) === 5000) {
                            _this.$swal('error', returnData.message, 'Error');
                        }
        ...
        editData: function (data, updateId, model = 'ModalVue', callback = false) {
            const _this = this;
                ...
                if (result.value) {
                    ...
                    _this.axios.delete(URL)
                        .then(function (response) {
                            ...
                            if (parseInt(returnData.status) === 2000) {
                                _this.$toastr('error', returnData.message, 'Success');
                                if (callDataList) {
                                    _this.getDataList();
                                }
                                if (typeof callback == 'function') {
                                ...
                                _this.$toastr('warning', returnData.message, 'Warning');
                            }
                        }).catch(function (error) {
